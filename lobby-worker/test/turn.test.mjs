...
test("allows one GET after the limiter and mints TURN credentials once", async () => {
  const calls = [];
  let upstreamCalls = 0;
  globalThis.fetch = async (input, init) => {
    upstreamCalls += 1;
    assert.equal(input, "https://rtc.live.cloudflare.com/v1/turn/keys/onedeck-key/credentials/generate-ice-servers");
  ...
  assert.deepEqual(calls, [{ key: "turn:203.0.113.7" }]);
  assert.equal(upstreamCalls, 1);
  assert.deepEqual(await response.json(), { iceServers: [{ urls: "stun:example.test" }] });
});

test("returns 429 and does not call the TURN API when the limiter refuses", async () => {
  let upstreamCalls = 0;
